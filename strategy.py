“””
ScalpingBot — Strategy v2
Momentum + Breakout scalping on 1m candles.
Designed for high-frequency signals on USDC pairs.
TP: 0.3% / SL: 0.15% — fast in, fast out.
“””
...
class Candle:
def **init**(self, o: float, h: float, l: float, c: float, v: float = 0):
self.open  = o
self.high  = h
self.low   = l
self.close = c
...
def **init**(self, pair: str, config: Config):
self.pair   = pair
self.config = config

```
    # EMA50 come trend filter su 1m (più reattiva di EMA200)
    self.ema_trend = EMA(50)
    self.ema_fast  = EMA(config.ema_fast)   # 9
    self.ema_slow  = EMA(config.ema_slow)   # 21
...
def _momentum_bull(self, candles: list) -> bool:
    """Candela bullish forte con body > 55% e chiusura sopra EMA9."""
    if len(candles) < 2:
        return False
    last = candles[-1]
...
def _breakout_bull(self, candles: list) -> bool:
    """Prezzo rompe il massimo delle ultime 5 candele con volume."""
    if len(candles) < 6:
        return False
    last  = candles[-1]
    prev5 = candles[-6:-1]
...
def _breakout_bear(self, candles: list) -> bool:
    """Prezzo rompe il minimo delle ultime 5 candele con volume."""
    if len(candles) < 6:
        return False
    last  = candles[-1]
    prev5 = candles[-6:-1]
...
def get_signal(self) -> SignalType:
    ...
    # Distanza minima EMA50 — più permissiva su 1m
    ...
    # Soglie OB/flow basse per massimizzare segnali su 1m
    ...
    # Debug log — una volta per candela
    if self._debug_logged_at != self._last_candle_time and (long_pattern or short_pattern):
        ...
        logger.debug(
            ...
            f"trend={'✅' if trend_ok else f'❌ dist={dist_pct:.3f}%'} "
