...
class Config:
    ...
    # Markets — default OFF, user decides from app
    ...
    ema_fast: int = 9
    ema_slow: int = 21
    ob_imbalance_threshold: float = 0.65
    ...
    spot_take_profit_pct: float = 0.008
    spot_stop_loss_pct: float = 0.004
    ...
    def load(cls) -> "Config":
        cfg = cls(
            ...
            spot_take_profit_pct=float(os.getenv("SPOT_TAKE_PROFIT_PCT", "0.008")),
            spot_stop_loss_pct=float(os.getenv("SPOT_STOP_LOSS_PCT", "0.004")),
        ...
        if state:
            ...
            logger.info(
                ...
                f"TP={cfg.take_profit_pct:.2%} SL={cfg.stop_loss_pct:.2%}"
            )
        else:
            from loguru import logger
